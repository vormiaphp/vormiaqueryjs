...
/**
 * Enhanced caching hook for Vue.js with vormiaqueryjs-style syntax
 * Provides easy-to-use caching operations with automatic refresh and management
 */
export const useVormiaCacheVue = (options = {}) => {
  const {
  ...
  const refreshTimers = ref(new Map());
  const retryCounters = ref(new Map());
  ...
  const refresh = async (key, refreshFunction, options = {}) => {
    ...
    try {
      ...
      // Handle retries
      const retryCount = retryCounters.value.get(key) || 0;
      if (retryCount < itemMaxRetries) {
        retryCounters.value.set(key, retryCount + 1);
  ...
  const refreshData = async (key, refreshFunction, options) => {
    ...
    if (result.success) {
      retryCounters.value.delete(key);
    }
  ...
  const setupAutoRefresh = (
    ...
    if (refreshTimers.value.has(key)) {
      clearTimeout(refreshTimers.value.get(key));
    }
    ...
    refreshTimers.value.set(key, timer);
  };
  ...
  const remove = (key) => {
    ...
    if (refreshTimers.value.has(key)) {
      clearTimeout(refreshTimers.value.get(key));
      refreshTimers.value.delete(key);
    }
  ...
  const invalidate = (pattern, options = {}) => {
    ...
    if (clearTimers) {
      ...
      refreshTimers.value.forEach((timer, key) => {
...
export default useVormiaCacheVue;
